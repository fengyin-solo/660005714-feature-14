  ...
  <div class="panel">
    ...
    <div v-for="(roi,i) in rois" :key="i" class="roi-config">
      <div class="roi-row">
        ...
        <el-input-number v-model="roi.center[0]" size="small" :min="0" :max="maxDim[0]-1" style="width:65px" controls-position="right"/>
        <el-input-number v-model="roi.center[1]" size="small" :min="0" :max="maxDim[1]-1" style="width:65px" controls-position="right"/>
        <el-input-number v-model="roi.center[2]" size="small" :min="0" :max="maxDim[2]-1" style="width:65px" controls-position="right"/>
    ...
    <div v-if="results.length" class="results">
      <div v-for="r in results" :key="r.label" class="roi-result">
...
// ROI 定义与结果按"当前这一份"存放，切换分组/聚焦再回来时测量配置仍在
const rois = computed<ROIDef[]>({
  get: () => store.roiDefs,
  set: v => { store.roiDefs = v },
})
const results = computed(() => store.roiResults)
const maxDim = computed<[number, number, number]>(() =>
  (store.volumeData?.dimensions ?? [64, 64, 64]) as [number, number, number])

function addROI() {
  rois.value.push({ label: `roi-${rois.value.length+1}`, center: [32, 32, 32], radius: 8 })
}
function removeROI(i: number) { rois.value.splice(i, 1) }
function analyze() { void store.analyzeROI() }
</script>
